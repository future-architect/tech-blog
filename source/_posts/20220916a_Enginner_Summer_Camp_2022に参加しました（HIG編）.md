...
__Future Medical Base__ とはフューチャーのコンポーネントをベースにHIG独自に開発したプラットフォームであり、これをもとに様々な医療向けアプリを構築できます。ちなみに、「Future Medical Base」は商標も取得されたそうです（おめでとうございます   :tada:）。
...
また、Merge Requestが通った時はエンジニアに仲間入りした気分でした。:laughing:
