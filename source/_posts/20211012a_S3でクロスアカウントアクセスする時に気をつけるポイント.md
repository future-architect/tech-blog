...
## 確実に--acl bucket-owner-full-controlを設定してもらう方法

`--acl bucket-owner-full-control` オプションを必須化するようなバケットポリシーを設定できます。
...
`--acl bucket-owner-full-control` が付与していない場合、書き込み時にエラーになるためより早く想定外の操作を検知することができます。
...
特にオブジェクト所有者の変更（`bucket-owner-full-control`の付与）は忘れやすく、「普段プログラムから書き込んでいるが、急遽CLIから書き込んでみたら読めなかった」ということが発生しないように覚えておくといいと思います。
