...
category:
  - Infrastructure
thumbnail: /images/xxxxxxxx/thumbnail_0XXX.jpg
author: "西田好孝"
featured: true
lede: "---"
---
...
# はじめに
こんにちは、TIGのDXユニットの西田です。前職ではServiceNowというaPaaS上でのアプリけーション開発をしておりました。現在は、GCPインフラの設計・構築をTerraform, Ansibleを利用して開発しております。GCP, ServiceNow ともに資格を持っています。
...
まず、**GCE の定義はこのメニュー化された 1 つの tf ファイルだけ**です。それを変数のリスト分だけ loop で回してリソースを作っています。[前原さんの記事](https://future-architect.github.io/articles/20190819/)を参考にしています。
...
type GCE_instance_request struct {
  Action      string `json:"action"`
  GCEInstance struct {
    GCP_project_name  string `json:"GCP_project_name"`
    GCE_instance_name string `json:"GCE_instance_name"`
  } `json:"GCE_instance"`
}

func main() {
  http.HandleFunc("/gce_instance", gce_instance)
  // http.HandleFunc("/gcs_bucket", gcs_bucket) // スペースの関係で一旦作りません…m(_ _)m
  http.ListenAndServe(":8080", nil)
}

// /gce_instance に POST された時に実行される関数
func gce_instance(w http.ResponseWriter, r *http.Request) {
  ...
  var gce_req GCE_instance_request
  if err = json.Unmarshal(b, &gce_req); err != nil {
    http.Error(w, err.Error(), 500)
    return
  }
  Action            := gce_req.Action
  GCP_project_name  := gce_req.GCEInstance.GCP_project_name
  GCE_instance_name := gce_req.GCEInstance.GCE_instance_name

  // Debug用
  fmt.Println("Action = "            + Action)
  fmt.Println("GCP_project_name = "  + GCP_project_name)
  fmt.Println("GCE_instance_name = " + GCE_instance_name)

  // plan の時だけ、gce 用の terraform の tfvars に追加する。
  if Action == "plan"{
    add_gce_tf_tfvars(GCP_project_name, GCE_instance_name)
  }

  strtemp := exec_tf(GCP_project_name, Action)
  fmt.Fprintf(w, strtemp)
}

// terraform の gce instance の tfvars にリクエストされた instance を追加する。
func add_gce_tf_tfvars(project string, gce_instance string) {

  tfvarpath   := TF_PATH + "/" + project + "/" + TF_VARDIR
  tfvars_file := TF_PATH + "/" + project + "/" + TF_VARDIR + "/" + TF_GCE_TFVARS

  raw, err := ioutil.ReadFile(tfvars_file)
  if err != nil {
    fmt.Println(err.Error())
    os.Exit(1)
  }
  fmt.Println(tfvars_file + " read success")
  ...
  // output to file
  file, err := os.Create(tfvars_file)
  if err != nil {
    // Openエラー処理
  }
  defer file.Close()
...
func exec_tf(project string, action string) (string) {
